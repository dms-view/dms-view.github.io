...
.domain(sitePreferenceData.map(d=>d.site))
.rangeRound([0, plotWidth])
.padding(0.05);

var yScale = d3.scaleLinear()
.domain([0, d3.max(sitePreferenceData, d=> d.value)]).nice()
.range([plotHeight,0]); //this is backwards because SVG 0,0 is at top left
...
.append("g")
.attr("transform","translate(" + margin.left + ", " + margin.top+")"); //this looks heinous because you're writing a string that should be printed into the HTML file.

//add an x axis to the plotting space
svg.append("g") //remember g is the keyword for group in the html file
.attr("class","x_axis")
.attr("transform", "translate(0," + plotHeight+ ")")// necessary to go from 0,0 being at the top to 0,0 being in the normal place, aka SVG coordinate space to Cartesian.
...
// Add an y-axis title.
svg.append("text")
   .attr("transform", "translate(" + (margin.left / -2) + ", " + (plotHeight / 2)  + ") rotate(-90)")
   .style("text-anchor", "middle")
   .text("Mutational tolerance");
...
.on("mousemove", function(d){return tooltip.style("top", (event.pageY-10)+"px").style("left",(event.pageX+10)+"px").text(" Mutational tolerance of "+ d.aminoAcid +": "+ d.value);})
