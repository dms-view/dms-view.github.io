...
var svg = d3.select("#line_plot").append("svg").attr("width", svg_dx).attr("height", svg_dy);
...
var brush = d3.brushX().extent([[0, 0], [plot_dx, plot_dy2]]).on("brush end", brushed);

// This defines the zoom. The zoom should on the min/max of the context plot?
var zoom = d3.zoom().scaleExtent([1, Infinity]).translateExtent([[0, 0], [plot_dx, plot_dy]]).extent([[0, 0], [plot_dx, plot_dy2]]).on("zoom", zoomed);
...
var tooltip = d3.select("#line_plot").append("div").style("font-family", "'Open Sans', sans-serif").style("position", "relative").style("font-size", "20px").style("z-index", "10").style("visibility", "hidden");

// Here is where we read in the data and create the plot
d3.csv("_data/line-data-PGT151.csv", function (d) {
  ...
  focus.append("path").data([d]).attr("class", "line").attr("d", valueline);

  var circlePoint = focus.append("g").selectAll("circle").data(d).enter().append("circle");

  var circleAttributes = circlePoint.attr("r", 5).attr("cx", function (d) {
    return x(+d.site);
  }).attr("cy", function (d) {
    return y(+d.abs_diffsel);
  }).attr("class", "non_brushed").on("mouseover", function (d) {
    return tooltip.style("visibility", "visible").text(d.site + d.abs_diffsel);
  }).on("mousemove", function (d) {
    return tooltip.style("top", event.pageY - 250 + "px").style("left", event.pageX - 600 + "px").text("Immune selection of " + d.site + ": " + d.abs_diffsel);
  }).on("mouseout", function (d) {
    return tooltip.style("visibility", "hidden");
  }).on("click", function (d) {
    xx = d.site;console.log(xx);
  });

  focus.append("g").attr("class", "axis axis--x").attr("id", "axis_x").attr("transform", "translate(0," + plot_dy + ")").call(xAxis);

  focus.append("g").attr("class", "axis axis--y").attr("id", "axis_y").call(yAxis);

  d3.select("#axis_x").append("text").attr("transform", "translate(1000, 10)").text("Site");

  d3.select("#axis_y").append("text").attr("transform", "rotate(-90) translate(-20, 15)").text("abs_diffsel Value");

  // make the smaller plot (called context in the tutorial)
  context.append("path").datum(d).attr("class", "area").attr("d", area2);

  context.append("g").attr("class", "axis axis--x").attr("transform", "translate(0," + plot_dy2 + ")").call(xAxis2);

  // add in the brush
  context.append("g").attr("class", "brush").call(brush).call(brush.move, x.range());

  svg.append("rect").attr("class", "zoom").attr("width", plot_dx).attr("height", plot_dy2).attr("transform", "translate(" + margin.left + "," + margin.top + ")").call(zoom);
});
